...
##
##
##  For recreating table 1 of Brimacombe et al. (2024):
...
each_average_distance_same_ecosystem <- matrix(,nrow=(length(levels(as.factor(metadata$Primary_type)))),ncol=3)
colnames(each_average_distance_same_ecosystem) <- c("Ecosystem_type","mean_distance","number_of_networks")

for (i in 1:nrow(each_average_distance_same_ecosystem)) {
  
  each_average_distance_same_ecosystem[i,1] <- levels(as.factor(metadata$Primary_type))[i]
...
for (i in 1:nrow(each_average_distance_same_ecosystem)) {
  ...
  for (j in 1:ncol(ecological)) {
    ...
    while (found != TRUE) {
      ...
      if (as.character(rowOfInterest$name) == network_name) {
        ...
        if (rowOfInterest$Primary_type != each_average_distance_same_ecosystem[i,1]) {
  ...
  each_average_distance_same_ecosystem[i,2] <- round(mean(ecological_intermediate[lower.tri(ecological_intermediate)]),2)
  
  #number of food webs that are of the specific ecosystem grouping of interest
  each_average_distance_same_ecosystem[i,3] <- ncol(ecological_intermediate)
  
}

each_average_distance_same_ecosystem <- as.data.frame(each_average_distance_same_ecosystem)
each_average_distance_same_ecosystem$mean_distance <- as.numeric(each_average_distance_same_ecosystem$mean_distance)
each_average_distance_same_ecosystem$number_of_networks <- as.numeric(each_average_distance_same_ecosystem$number_of_networks)

##
##  Mean pairwise GCD-11 between "terrestrial" food webs, 
##  with "Digel et al. (2014)" food webs removed
##

#storage for mean pairwise GCD-11 between food webs from the same ecosystem
each_average_distance_terrestrial_no_digel <- matrix(,nrow=1,ncol=3)
colnames(each_average_distance_terrestrial_no_digel) <- c("Ecosystem_type","mean_distance","number_of_networks")
each_average_distance_terrestrial_no_digel[1,1] <- "Terrestrial"

#looping through the pairwise GCD-11 matrix to find food webs associated 
#with specific ecosystems
for (i in 1:nrow(each_average_distance_terrestrial_no_digel)) {
  ...
  for (j in 1:ncol(ecological)) {
    ...
    while (found != TRUE) {
      ...
      if (as.character(rowOfInterest$name) == network_name) { 
        ...
        if (rowOfInterest$Primary_type != each_average_distance_terrestrial_no_digel[i,1]) {
  ...
  #mean pairwise GCD-11 between specific ecosystem's food webs
  each_average_distance_terrestrial_no_digel[i,2] <- round(mean(ecological_intermediate[lower.tri(ecological_intermediate)]),2)
  
  #number of food webs that are of the specific ecosystem grouping of interest
  each_average_distance_terrestrial_no_digel[i,3] <- ncol(ecological_intermediate)
...
each_average_distance_aquatic_v_aquatic_terrestrial <- ecological[-c(rows_columns_delete,rows_delete),-c(rows_columns_delete,columns_delete)]
...
each_average_distance_aquatic_v_terrestrial <- ecological[-c(rows_columns_delete,rows_delete),-c(rows_columns_delete,columns_delete)]
...
each_average_distance_aquatic_terrestrial_v_terrestrial <- ecological[-c(rows_columns_delete,rows_delete),-c(rows_columns_delete,columns_delete)]

##
##  All information for table 1
##

#mean pairwise GCD-11 between food webs from the same ecosystem
each_average_distance_same_ecosystem

#mean pairwise GCD-11 between "terrestrial" food webs with Digel et al. (2014) removed
each_average_distance_terrestrial_no_digel

#mean pairwise GCD-11 between "aquatic" and "aquatic and terrestrial" food webs
round(mean(each_average_distance_aquatic_v_aquatic_terrestrial),2)

#mean pairwise GCD-11 between "aquatic" and "terrestrial" food webs
round(mean(each_average_distance_aquatic_v_terrestrial),2)

#mean pairwise GCD-11 between "aquatic and terrestrial" and "terrestrial" food webs
round(mean(each_average_distance_aquatic_terrestrial_v_terrestrial),2)
